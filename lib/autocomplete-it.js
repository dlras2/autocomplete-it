...
class AI {
  ...
  static highlightMatch(match, mapping, open, close) {
    ...
    const spans = map
      ? AI.getMappedHighlightedSpans(match, value, map)
      : AI.getHighlightedSpans(match, value);
    const display = AI.highlightSpans(value, spans, open, close);
    const result = { display, value };
    return result;
  }

  static getMappedHighlightedSpans(match, value, map) {
